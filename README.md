...
❗현 프로젝트는 `data/processed`에 있는 2026-03-01 ~ 2026-08-31 데이터를 Firestore Database에 새로 등록된 9월 데이터와 합쳐 요약을 제공하는 방식으로 구현되어 있다. 이는 Firestore에 데이터가 정상적으로 저장되고 있음을 확인하는 동시에, Firestore 무료 티어의 쿼터(일일 쓰기 2만 건 / 읽기 5만 건)를 절약하기 위함이다. 또한 초기 로딩 속도가 빠르고 요약 연산도 효율적으로 수행된다. 실무 환경에서는 일반적으로 모든 데이터를 Firestore에 저장하여 관리한다.
